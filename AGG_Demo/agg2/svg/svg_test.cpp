...
public:
    ...
    virtual void on_draw()
    {
        ...
        mtx *= agg::translation_matrix((m_min_x + m_max_x) * -0.5, (m_min_y + m_max_y) * -0.5);
        mtx *= agg::scaling_matrix(m_scale.value());
        mtx *= agg::rotation_matrix(agg::deg2rad(m_rotate.value()));
        mtx *= agg::translation_matrix((m_min_x + m_max_x) * 0.5 + m_x, (m_min_y + m_max_y) * 0.5 + m_y + 30);
    ...
    virtual void on_key(int x, int y, unsigned key, unsigned flags)
    {
        if(key == ' ')
        {
            ...
            mtx *= agg::rotation_matrix(agg::deg2rad(m_rotate.value()));
            mtx *= agg::translation_matrix((m_min_x + m_max_x) * 0.5, (m_min_y + m_max_y) * 0.5);
            mtx *= agg::translation_matrix(m_x, m_y);
