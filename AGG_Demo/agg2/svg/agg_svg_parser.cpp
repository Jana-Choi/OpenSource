...
namespace svg
{
    ...
    bool parser::parse_attr(const char* name, const char* value)
    {
        ...
        {
            if(strcmp(value, "butt") == 0)        m_path.line_cap(gen_stroke::butt_cap);
            else if(strcmp(value, "round") == 0)  m_path.line_cap(gen_stroke::round_cap);
            else if(strcmp(value, "square") == 0) m_path.line_cap(gen_stroke::square_cap);
        }
        else
        if(strcmp(name, "stroke-linejoin") == 0)
        {
            if(strcmp(value, "miter") == 0)      m_path.line_join(gen_stroke::miter_join);
            else if(strcmp(value, "round") == 0) m_path.line_join(gen_stroke::round_join);
            else if(strcmp(value, "bevel") == 0) m_path.line_join(gen_stroke::bevel_join);
        }
        else
    ...
    unsigned parser::parse_matrix(const char* str)
    {
        ...
        m_path.transform() *= affine_matrix(args[0], args[1], args[2], args[3], args[4], args[5]);
        return len;
    }
    ...
    unsigned parser::parse_translate(const char* str)
    {
        ...
        if(na == 1) args[1] = 0.0;
        m_path.transform() *= translation_matrix(args[0], args[1]);
        return len;
    }
    ...
    unsigned parser::parse_rotate(const char* str)
    {
        ...
        if(na == 1) 
        {
            m_path.transform() *= rotation_matrix(deg2rad(args[0]));
        }
        else if(na == 3)
        {
            m_path.transform() *= translation_matrix(-args[1], -args[2]);
            m_path.transform() *= rotation_matrix(deg2rad(args[0]));
            m_path.transform() *= translation_matrix(args[1], args[2]);
        }
        else
    ...
    unsigned parser::parse_scale(const char* str)
    {
        ...
        m_path.transform() *= scaling_matrix(args[0], args[1]);
        return len;
    }
    ...
    unsigned parser::parse_skew_x(const char* str)
    {
        ...
        m_path.transform() *= skewing_matrix(arg, 0.0);
        return len;
    }
    ...
    unsigned parser::parse_skew_y(const char* str)
    {
        ...
        unsigned na = 0;
        unsigned len = parse_transform_args(str, &arg, 1, &na);
        m_path.transform() *= skewing_matrix(0.0, arg);
        return len;
    }
