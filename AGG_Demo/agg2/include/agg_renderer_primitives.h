...
namespace agg
{
    ...
    public:
        ...
        {
            m_ren->blend_hline(x1,   y1,   x2-1, m_line_color, color_type::opaque());
            m_ren->blend_vline(x2,   y1,   y2-1, m_line_color, color_type::opaque());
            m_ren->blend_hline(x1+1, y2,   x2,   m_line_color, color_type::opaque());
            m_ren->blend_vline(x1,   y1+1, y2,   m_line_color, color_type::opaque());
        }

        //--------------------------------------------------------------------
        void solid_rectangle(int x1, int y1, int x2, int y2)
        {
            m_ren->blend_bar(x1, y1, x2, y2, m_fill_color, color_type::opaque());
        }
        ...
        void outlined_rectangle(int x1, int y1, int x2, int y2) 
        {
            ...
            m_ren->blend_bar(x1+1, y1+1, x2-1, y2-1, m_fill_color, color_type::opaque());
        }
        ...
        void ellipse(int x, int y, int rx, int ry)
        {
            ...
            do
            {
                dx += ei.dx();
                dy += ei.dy();
                m_ren->blend_pixel(x + dx, y + dy, m_line_color, color_type::opaque());
                m_ren->blend_pixel(x + dx, y - dy, m_line_color, color_type::opaque());
                m_ren->blend_pixel(x - dx, y - dy, m_line_color, color_type::opaque());
                m_ren->blend_pixel(x - dx, y + dy, m_line_color, color_type::opaque());
                ++ei;
        ...
        void solid_ellipse(int x, int y, int rx, int ry)
        {
            ...
            do
            {
                ...
                if(dy != dy0)
                {
                    m_ren->blend_hline(x-dx0, y+dy0, x+dx0, m_fill_color, color_type::opaque());
                    m_ren->blend_hline(x-dx0, y-dy0, x+dx0, m_fill_color, color_type::opaque());
                }
                dx0 = dx;
                dy0 = dy;
        ...
        void outlined_ellipse(int x, int y, int rx, int ry)
        {
            ...
            do
            {
                ...
                if(ei.dy() && dx)
                {
                   m_ren->blend_hline(x-dx+1, y+dy, x+dx-1, m_fill_color, color_type::opaque());
                   m_ren->blend_hline(x-dx+1, y-dy, x+dx-1, m_fill_color, color_type::opaque());
                }
                ++ei;
            }
        ...
        void line(int x1, int y1, int x2, int y2, bool last=false)
        {
            ...
            if(len == 0)
            {
                if(last)
                {
                    m_ren->blend_pixel(li.line_lr(x1), li.line_lr(y1), m_line_color, color_type::opaque());
                }
                return;
            }
            ...
            if(li.is_ver())
            {
                do
                {
                    m_ren->blend_pixel(li.x2(), li.y1(), m_line_color, color_type::opaque());
                    li.vstep();
                }
                while(--len);
            }
            else
            {
                do
                {
                    m_ren->blend_pixel(li.x1(), li.y2(), m_line_color, color_type::opaque());
                    li.hstep();
                }
                while(--len);
            }
