...
namespace agg
{
    ...
    public:
        ...
        //--------------------------------------------------------------------
        // Auxiliary functions. resizing_matrix() modifier sets up the resizing 
        ...
        // resizing_matrix() accessor simply returns current resizing matrix 
        ...
        void resizing_matrix(int width, int height)
        {
            if(m_window_flags & window_keep_aspect_ratio)
            {
                ...
                if(sy < sx) sx = sy;
                m_resize_mtx = scaling_matrix(sx, sx);
            }
            else
            {
                m_resize_mtx = scaling_matrix(
                    double(width) / double(m_initial_width),
                    double(height) / double(m_initial_height));
            }
        }
        const affine_matrix& resizing_matrix() const { return m_resize_mtx; }
