...
public:
    ...
    template<class Scanline> void render(const Scanline& sl)
    {
        ...
        do
        {
            unsigned span_len = span->len;
            int x;
            if(span->pix_covers)
            {
                // pixel-by-pixel
                const Scanline::cover_type* covers = span->pix_covers;
                x = span->x;
                do
                {
            ...
            {
                if(span->span_cover == 255)
                {
                    // solid span. happens often
                    draw_solid_span(span->x, y, span_len);
                }
                else
                {
                    // pixel-by-pixel again
                    unsigned cover = span->span_cover;
                    x = span->x;
                    do
                    {
                        blend_pixel(x++, y, cover);
                    }
                    while(--span_len);
                }
            }
...
void CCustomRendererView::OnDraw(CDC* pDC)
{
    ...
    mtx *= agg::rotation_matrix(g_angle);
