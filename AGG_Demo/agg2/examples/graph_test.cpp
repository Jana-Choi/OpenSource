...
typedef agg::span_gradient<agg::rgba8, interpolator, gradient_function> gradient_span_gen;
...
{
    typedef agg::conv_marker_adaptor<Source, agg::gen_markers_term>             stroke_type;
...
{
    typedef agg::conv_stroke<Source, agg::gen_markers_term>                     stroke_type;
    typedef agg::conv_marker<typename stroke_type::marker_type, agg::arrowhead> marker_type;
    typedef agg::conv_concat<stroke_type, marker_type>                          concat_type;
...
{
    typedef agg::conv_dash<Source, agg::gen_markers_term> dash_type;
        ...
        for(i = 0; i < m_graph.get_num_nodes(); i++)
        {
            ...
            switch(m_draw)
            {
                ...
                case 3:
                {
                    ...
                    mtx *= agg::scaling_matrix(m_width.value() / 2.0);
                    mtx *= agg::translation_matrix(n.x, n.y);
                    mtx.invert();
