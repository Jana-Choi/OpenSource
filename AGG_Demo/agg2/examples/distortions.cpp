...
public:
    ...
    virtual void on_draw()
    {
        ...
        src_mtx *= agg::translation_matrix(-img_width/2, -img_height/2);
        src_mtx *= agg::rotation_matrix(m_angle.value() * agg::pi / 180.0);
        src_mtx *= agg::translation_matrix(img_width/2 + 10, img_height/2 + 10 + 40);
        src_mtx *= resizing_matrix();

        agg::affine_matrix img_mtx;
        img_mtx *= agg::translation_matrix(-img_width/2, -img_height/2);
        img_mtx *= agg::rotation_matrix(m_angle.value() * agg::pi / 180.0);
        img_mtx *= agg::scaling_matrix(m_scale.value());
        img_mtx *= agg::translation_matrix(img_width/2 + 10, img_height/2 + 10 + 40);
        img_mtx *= resizing_matrix();
        img_mtx.invert();
...
*/
        ...
        src_mtx *= ~resizing_matrix();
        src_mtx *= agg::translation_matrix(img_width - img_width/10, 0.0);
        src_mtx *= resizing_matrix();
        ...
        typedef agg::span_gradient<agg::rgba8, 
                                   interpolator_type,
                                   agg::gradient_circle> gradient_span_gen;
        ...
        agg::affine_matrix gr1_mtx;
        gr1_mtx *= agg::translation_matrix(-img_width/2, -img_height/2);
        gr1_mtx *= agg::scaling_matrix(0.8);
        gr1_mtx *= agg::rotation_matrix(m_angle.value() * agg::pi / 180.0);
        gr1_mtx *= agg::translation_matrix(img_width - img_width/10 + img_width/2 + 10, 
                                          img_height/2 + 10 + 40);
        gr1_mtx *= resizing_matrix();

        agg::affine_matrix gr2_mtx;
        gr2_mtx *= agg::rotation_matrix(m_angle.value() * agg::pi / 180.0);
        gr2_mtx *= agg::scaling_matrix(m_scale.value());
        gr2_mtx *= agg::translation_matrix(img_width - img_width/10 + img_width/2 + 10 + 50, 
                                           img_height/2 + 10 + 40 + 50);
        gr2_mtx *= resizing_matrix();
        gr2_mtx.invert();
